...
final class FakeEventConsumer implements ContractConsumer
{
    ...
    /** @var list<array{message: ContractMessage, failure: string}> */
    public array $failed = [];
    ...
    public function fail(ContractMessage $message, string $failure): void
    {
...
function createContractEventWorker(
    ...
    $consumer = new FakeEventConsumer([new ContractMessage('message-1', 'test-payload')]);
...
$runContractWorker('EventWorker 将 listener 失败交给 Consumer', static function () use (
    ...
    $failure = json_decode($context['consumer']->failed[0]['failure'], true, flags: JSON_THROW_ON_ERROR);
    $contractWorkerAssertSame('message-1', $failure['message_id'], '失败摘要泄漏了具体通道命名');
    $contractWorkerAssertSame(false, $failure['listeners'][0]['succeeded'], '失败摘要缺少 listener 结果');
});
...
$runContractWorker('EventWorker 将反序列化失败交给 Consumer', static function () use (
    ...
    $failure = json_decode($context['consumer']->failed[0]['failure'], true, flags: JSON_THROW_ON_ERROR);
    $contractWorkerAssertSame(RuntimeException::class, $failure['error_class'], '反序列化异常类型未保留');
    $contractWorkerAssertSame('message-1', $failure['message_id'], '失败摘要消息 ID 错误');
});
